...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/phoenixxenon6/XENONLO/main/"
...
echo -e "\e[32mloading...\e[0m" 
clear
izinsc="https://raw.githubusercontent.com/phoenixxenon6/XENONLO/main/keygen"
# USERNAME
rm -f /usr/bin/user
...
print_install "Configuring SSHD"
    ...
    # Prompt for a safe SSH port (not between 222-1000)
    default_port=22222
    read -p "Enter SSH Port (outside 222-1000) [${default_port}]: " SSH_PORT
    SSH_PORT=${SSH_PORT:-$default_port}
    while true; do
        if [[ "$SSH_PORT" =~ ^[0-9]+$ ]] && { [ "$SSH_PORT" -le 221 ] || [ "$SSH_PORT" -ge 1001 ]; }; then
            break
        fi
        echo "Invalid port. Must be <=221 or >=1001."
        read -p "Enter SSH Port [${default_port}]: " SSH_PORT
        SSH_PORT=${SSH_PORT:-$default_port}
    done

    # Update Port directive
    ...
    # Enable key and password authentication
    ...
    sed -i 's/^\s*ChallengeResponseAuthentication\s\+.*/ChallengeResponseAuthentication no/' "$sshd_config" 2>/dev/null || true
    grep -q '^ChallengeResponseAuthentication' "$sshd_config" || echo 'ChallengeResponseAuthentication no' >> "$sshd_config"
    sed -i 's/^\s*UsePAM\s\+.*/UsePAM yes/' "$sshd_config" 2>/dev/null || true
    grep -q '^UsePAM' "$sshd_config" || echo 'UsePAM yes' >> "$sshd_config"

    # AllowUsers for extra security (space-separated)
    read -p "Enter allowed SSH usernames (space-separated), or leave empty to skip: " ALLOW_USERS
    if [ -n "$ALLOW_USERS" ]; then
        sed -i '/^AllowUsers\b/d' "$sshd_config"
        echo "AllowUsers ${ALLOW_USERS}" >> "$sshd_config"
    fi

    chmod 600 "$sshd_config"

    # Open firewall for the chosen SSH port
...
/etc/init.d/ssh status
    print_success "SSHD configured on port ${SSH_PORT}"
}

# Generate SSH keys for root and install public key (with or without passphrase)
function setup_ssh_keys(){
clear
print_install "Generating SSH key pair"

    mkdir -p /root/.ssh
    chmod 700 /root/.ssh

    default_key_type="ed25519"
    read -p "Choose key type [ed25519|rsa] (default: ${default_key_type}): " KEY_TYPE
    ...
    if [ "$KEY_TYPE" = "rsa" ]; then
        KEYGEN_CMD="ssh-keygen -t rsa -b 4096"
        KEY_NAME="id_rsa"
    else
        KEYGEN_CMD="ssh-keygen -t ed25519"
        KEY_NAME="id_ed25519"
    fi
    ...
    if [ -f "$KEY_PATH" ]; then
        ...
        read -p "Enter key passphrase (leave empty for passless): " KEY_PASSPHRASE
        if [ -n "$KEY_PASSPHRASE" ]; then
            printf "%s\n%s\n" "$KEY_PASSPHRASE" "$KEY_PASSPHRASE" | eval $KEYGEN_CMD -f "$KEY_PATH" -C "root@$(hostname)" >/dev/null 2>&1
        else
            eval $KEYGEN_CMD -f "$KEY_PATH" -N '' -C "root@$(hostname)" >/dev/null 2>&1
        fi
        chmod 600 "$KEY_PATH"
    ...
    print_success "SSH keys ready at ${KEY_PATH}"
}
